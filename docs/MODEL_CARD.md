...
### Primary use
Classifying short news text — headlines and short articles up to 128 BERT tokens — as **Real** or **Fake** for screening / triage purposes in English, Urdu, or Spanish.
...
- Languages beyond English, Urdu, and Spanish (model has no signal for them).
...
| Metric | Value |
|---|---|
| Overall accuracy (test set) | **90%** |
| Per-language accuracy | Reported by `evaluate.py` on your test CSV |

Reproduce on your own labelled CSV:
```bash
python evaluate.py --csv data/your_test.csv
```
...
Multi-source corpus covering English, Urdu, and Spanish news text balanced across the Real / Fake classes. Curation, sources, and split sizes are documented separately by the project author. If you fine-tune further on your own corpus, update this section.
...
- **Language coverage is sparse.** Within-language performance varies because training data is uneven across English / Urdu / Spanish. Per-language metrics in `evaluation_results.json` make this visible.
...
- **Transparency to end users.** If you embed this model in a product that surfaces predictions, label them as model-generated and disclose accuracy bounds (90% means roughly one wrong call in ten).
...
- Per-language slice metrics (English / Urdu / Spanish) reported separately so cross-language averages don't hide language-level regressions.
- See [`evaluate.py`](../evaluate.py) for the exact implementation.

## Caveats and recommendations

- Run `evaluate.py` on a sample of your own current data before relying on the 90% headline number — distribution drift between the training corpus and your live traffic is the most common reason published numbers don't hold.
